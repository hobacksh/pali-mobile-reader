...
PY_SCRIPT="${SCRIPT_DIR}/translate_vin01_trans_batches.py"

usage() {
  cat <<'EOF'
Usage:
  ./scripts/run_vin01_translation.sh --items <N> [--start-line <LINE>] [--batch-size 5] [--sleep-seconds 2] [--model gpt-5.3-codex] [--dry-run]

Examples:
  ./scripts/run_vin01_translation.sh --items 10
  ./scripts/run_vin01_translation.sh --items 20 --start-line 2501
  ./scripts/run_vin01_translation.sh --items 10 --dry-run
EOF
}
